<?php
    ...
    if (isset($_POST['ajouter']) && isset($_FILES['pdfFile'])) {
        $dossierPDF = '../include/pdf/adultes/';
        ...
        if($reqVerif->rowCount() > 0) {
            ...
            header('Location: ../adultes-familles.php?msgErreur=PDF déjà existante.');
            exit;
        }
        ...
        if ($extensionFichier == 'pdf') {
            ...
            if (move_uploaded_file($_FILES['pdfFile']['tmp_name'], $cheminFichier)) {
                ...
                $requeteInsert->execute(array(':pFileNamePDF' => $nomFichier, ':pFilePathPDF' => "./include/pdf/adultes/"));
                $requeteInsert->closeCursor();

                echo('<script language="JavaScript" type="text/javascript"> 
                    alert("Ajout du PDF effectué."); 
                    location.href = "../adultes-familles.php";
                </script>');
                exit;

            } else {
                header('Location: ../adultes-familles.php?msgErreur=Une erreur s\'est produite lors du téléchargement du PDF.');
                exit;
            }
        } else {
            header('Location: ../adultes-familles.php?msgErreur=Seuls les fichiers PDFs sont autorisés.');
            exit;
        }
    }
