<?php
    ...
    if (isset($_POST['ajouter']) && isset($_FILES['pdfFile'])) {
        $dossierPDF = '../include/pdf/benevoles/';
        ...
        if($reqVerif->rowCount() > 0) {
            ...
            header('Location: ../formations.php?msgErreur=PDF déjà existante.');
            exit;
        }
        ...
        if ($extensionFichier == 'pdf') {
            ...
            if (move_uploaded_file($_FILES['pdfFile']['tmp_name'], $cheminFichier)) {
                ...
                $requeteInsert->execute(array(':pFileNamePDF' => $nomFichier, ':pFilePathPDF' => "./include/pdf/benevoles/"));
                ...
                header('Location: ../formations.php?msgErreur=Une erreur s\'est produite lors du téléchargement du PDF.');
                exit;
            }
        } else {
            header('Location: ../formations.php?msgErreur=Seuls les fichiers PDFs sont autorisés.');
            exit;
        }
    }
