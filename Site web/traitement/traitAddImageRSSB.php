<?php
    ...
    if (isset($_POST['ajouter']) && isset($_FILES['monImage'])) {
        ...
        $dossierImages = '../include/images/imagesrssb/';
        $nomFichier = $_FILES['monImage']['name'];
        $descriptionImage = htmlentities($_POST['description']);
        ...
        if($reqVerif->rowCount() > 0) {
            ...
            header('Location: ../imagesrssb.php?msgErreur=Image déjà existante.');
            exit;
        }
        ...
        if (in_array($extensionFichier, $extensionsAutorisees)) {
            ...
            if (move_uploaded_file($_FILES['monImage']['tmp_name'], $cheminFichier)) {
                ...
                $requeteInsert->execute(array(':pNomImage' => $nomFichier, ':pCheminImage' => "./include/images/imagesrssb/", ':pDescriptionImage' => $descriptionImage));
                $requeteInsert->closeCursor();

                echo('<script language="JavaScript" type="text/javascript"> 
                    alert("Ajout effectué."); 
                    location.href = "../imagesrssb.php";
                </script>');
                exit;

            } else {
                header('Location: ../imagesrssb.php?msgErreur=Une erreur s\'est produite lors du téléchargement de l\'image.');
                exit;
            }
        } else {
            header('Location: ../imagesrssb.php?msgErreur=Seuls les fichiers JPG, JPEG et PNG sont autorisés.');
            exit;
        }
    }
